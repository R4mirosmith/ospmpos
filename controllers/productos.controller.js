...
const { serializeColors, productWithParsedColors, attachProductImages } = require('../utils/productFields');
...
function ocultarValoresAdministrativos(producto) {
  ...
  const safe = productWithParsedColors(producto);
  delete safe.costo;
...
function skuTimestamp(date = new Date()) {
  const pad = (n, len = 2) => String(n).padStart(len, '0');
  return `${String(date.getUTCFullYear()).slice(-2)}${pad(date.getUTCMonth() + 1)}${pad(date.getUTCDate())}${pad(date.getUTCHours())}${pad(date.getUTCMinutes())}${pad(date.getUTCSeconds())}${pad(date.getUTCMilliseconds(), 3)}`;
}

function generatedSku(nombre, attempt = 0) {
  const extra = attempt > 0 ? `-${String(attempt).padStart(2, '0')}` : '';
  return `${skuBase(nombre)}-${skuTimestamp()}${extra}`.slice(0, 80);
}
...
async function skuExists(db, sedeId, codigo, excludeId = null) {
  const params = [Number(sedeId), String(codigo)];
  let sql = `SELECT id FROM producto WHERE sede_id=? AND codigo=?`;
...
async function resolveUniqueSku(db, sedeId, nombre, requested, autoGenerated = false, excludeId = null) {
  let codigo = String(requested || '').trim();
  if (codigo.length > 80) {
    throw Object.assign(new Error('El SKU / código puede tener máximo 80 caracteres'), { httpStatus: 400, code: 'SKU_INVALIDO' });
  }

  if (codigo && !(await skuExists(db, sedeId, codigo, excludeId))) return codigo;
  if (codigo && !autoGenerated) {
    throw Object.assign(new Error(`El SKU / código “${codigo}” ya existe en esta sede`), { httpStatus: 409, code: 'SKU_DUPLICADO' });
  }

  for (let attempt = 0; attempt < 25; attempt++) {
    codigo = generatedSku(nombre, attempt);
    if (!(await skuExists(db, sedeId, codigo, excludeId))) return codigo;
  }

  throw Object.assign(new Error('No fue posible generar un SKU único. Intenta nuevamente.'), { httpStatus: 409, code: 'SKU_NO_DISPONIBLE' });
}
...
async function listar(req, res) {
  ...
  const normalized = items.map(productWithParsedColors);
  ok(res, { items: esGestorWeb(req) ? normalized.map(ocultarValoresAdministrativos) : normalized, total: Number(cnt.total || 0) });
}
async function get(req, res) {
  ...
  const normalized = productWithParsedColors(producto);
  ok(res, esGestorWeb(req) ? ocultarValoresAdministrativos(normalized) : normalized);
}
async function crear(req, res) {
  const { categoria_id, codigo, nombre, descripcion = '', garantia_info = null, colores_disponibles = null, costo = null, costo_promedio = null, precio = 0, precio_m = null, stock_minimo = 0, costoInicial = null, codigo_barras = null, codigo_autogenerado = 0 } = req.body || {};
  if (!categoria_id || !nombre) return badRequest(res, 'categoria_id y nombre requeridos');
  ...
  try {
    ...
    const coloresValor = serializeColors(colores_disponibles);
    const codigoFinal = await resolveUniqueSku(conn, s.sede_id, nombre, codigo, truthy(codigo_autogenerado) || !String(codigo || '').trim());

    const [r] = await conn.query(
      `INSERT INTO producto(empresa_id,sede_id,categoria_id,codigo,nombre,descripcion,garantia_info,colores_disponibles,costo,precio,precio_m,stock_minimo,activo,codigo_barras,fecha)
       VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,UTC_TIMESTAMP())`,
      [s.empresa_id, s.sede_id, categoria_id, codigoFinal, nombre, descripcion, garantia_info, coloresValor, cost, precioValor, precioMValor, stockMinimoValor, 1, codigo_barras || codigoFinal]
    );
    ...
    await conn.commit();
    created(res, { id: r.insertId, codigo: codigoFinal, message: 'Producto creado' });
...
async function crearConImagenes(req, res) {
  ...
  const {
    categoria_id, codigo, nombre, descripcion = '', garantia_info = null, colores_disponibles = null,
    costo = null, costo_promedio = null, precio = 0, precio_m = null,
    stock_minimo = 0, costoInicial = null, codigo_barras = null, codigo_autogenerado = 0,
  } = req.body || {};
  if (!categoria_id || !nombre) return badRequest(res, 'categoria_id y nombre requeridos');
  ...
  try {
    ...
    const coloresValor = serializeColors(colores_disponibles);
    const codigoFinal = await resolveUniqueSku(conn, scope.sede_id, nombre, codigo, truthy(codigo_autogenerado) || !String(codigo || '').trim());

    const [r] = await conn.query(
      `INSERT INTO producto(empresa_id,sede_id,categoria_id,codigo,nombre,descripcion,garantia_info,colores_disponibles,costo,precio,precio_m,stock_minimo,activo,codigo_barras,fecha)
       VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,UTC_TIMESTAMP())`,
      [scope.empresa_id, scope.sede_id, Number(categoria_id), codigoFinal, nombre, descripcion, garantia_info || null, coloresValor, cost, precioValor, precioMValor, stockMinimoValor, 1, codigo_barras || codigoFinal]
    );
    ...
    await conn.commit();
    created(res, { id: productoId, codigo: codigoFinal, message: 'Producto creado', uploaded, items: uploaded });
  } catch (e) {
    await conn.rollback();
...
async function actualizar(req, res) {
  ...
  if (body.colores_disponibles !== undefined) body.colores_disponibles = serializeColors(body.colores_disponibles);
  if (body.codigo !== undefined) {
    body.codigo = await resolveUniqueSku(pool, producto.sede_id, body.nombre || '', body.codigo, false, id);
  }
  ...
  const fields = ['categoria_id','codigo','nombre','descripcion','garantia_info','colores_disponibles','costo','precio','precio_m','stock_minimo','codigo_barras','activo'];
...
async function publicCatalogo(req, res) {
  ...
  const [items] = await pool.query(
    `SELECT p.id, p.codigo, p.codigo_barras, p.nombre, p.descripcion, p.garantia_info, p.colores_disponibles,
            ...
            p.video_url, p.video_duracion_segundos, p.video_mime, ${stockExpr('p')} stock
       FROM producto p
       LEFT JOIN categoria c ON c.id=p.categoria_id
      WHERE p.sede_id=? AND p.activo=1 ORDER BY p.nombre`, [sedeId]);
  const enriched = await attachProductImages(pool, items);
  ok(res, { total: enriched.length, items: enriched });
}
